...
Suppose `attempt_01/training.out` stopped at the SCF cycle limit. From its parent directory:

```bash
mkdir attempt_02
cp attempt_01/training.d12 attempt_02/training.d12
cp attempt_01/training.qsub attempt_02/training.qsub  # only if this route is used
cp attempt_01/run-note.txt attempt_02/run-note.txt
cd attempt_02
vi training.d12
```

Change one control only. Then check exactly what changed:

```bash
diff -u ../attempt_01/training.d12 training.d12
```

The diff should contain only the change described in the recovery note. Submit with the same approved route used for the first run:

```bash
qcry23 training.d12 <cores> <walltime>
```
...
Angle-bracketed values are placeholders. Copy the current resource format from the validated group command; do not type the brackets.

### Decide what the output pattern means

```bash
grep -n -i "cycle\|energy\|converg\|maxcycle\|error" training.out
tail -n 100 training.out
```

Read several final iterations rather than only the last energy. A slow, steady approach, a repeated oscillation and an abrupt numerical failure point to different problems. If the structure, charge or restart source is doubtful, fix that evidence before tuning more SCF controls.
