...
Use the official [CRYSTAL Basis Sets Library](https://www.crystal.unito.it/basis_sets.html) to see which basis sets are available for each element and which predefined sets can be called with `BASISSET`.[^2] Availability does not make a basis automatically suitable: begin with a basis already validated for the material and method, then test whether the result of interest is converged with respect to that choice.
...
### Try this on the training run

Return to the directory created in the previous page:
...
In `less`, search for a keyword with `/KEYWORD`, move to the next match with `n`, and quit with `q`. Then inspect the output:
...
Write the line numbers of the final SCF status and energy in `run-note.txt`. If you cannot find an explicit result, do not mark the run as converged.
...
The path is part of the physical interpretation. For bulk and two-dimensional slabs, use the reciprocal-space convention appropriate to the model and record the high-symmetry points in the calculation note. [CrySPLOT](https://crysplot.crystalsolutions.eu/index.html) can help you inspect the resulting data, but it cannot decide whether the path is physically appropriate.[^3]
...
[^2]: CRYSTAL. (n.d.). *Basis Sets Library*. https://www.crystal.unito.it/basis_sets.html
[^3]: CRYSTAL Solutions. (n.d.). *CrySPLOT*. https://crysplot.crystalsolutions.eu/index.html
