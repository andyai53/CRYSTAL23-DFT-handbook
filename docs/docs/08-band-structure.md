...
Keep the properties run separate from the parent calculation. From the parent project, the file-management part looks like this:

```bash
mkdir -p properties/band_01
cp relaxed.d12 properties/band_01/parent.d12
cp relaxed.out properties/band_01/parent.out
cp fort.9 properties/band_01/
cp /path/to/working-template/band.d3 properties/band_01/band-template.d3
cp properties.qsub properties/band_01/
cd properties/band_01
cp band-template.d3 band.d3
ls -lah
```

Use the restart filenames required by your local template. If the parent run produced `fort.98` or another required file, copy it from the same parent directory. Then:
...
4. Check the properties executable and submit the job through the local PBS wrapper.
5. Keep the `.d3`, submission script, output and generated data together.

Before submitting, inspect the exact changes and command:

```bash
diff -u band-template.d3 band.d3
grep -n "runcryP\|runpropP\|properties" properties.qsub
qsub properties.qsub
qstat -u "$USER"
```

The project note uses `runpropP` for a properties job. This is a local wrapper convention, so follow the current working template if its command differs. After the job leaves the queue:
...
Only open the result in CrySPLOT after the output confirms that the requested BAND analysis completed.
