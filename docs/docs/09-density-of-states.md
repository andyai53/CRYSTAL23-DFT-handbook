...
Create a new properties directory from the validated parent record. Copy the matching restart files and a working local DOSS template:

```bash
mkdir -p properties/doss_01
cp /path/to/accepted-parent/parent.d12 properties/doss_01/
cp /path/to/accepted-parent/parent.out properties/doss_01/
cp /path/to/accepted-parent/fort.9 properties/doss_01/
cp /path/to/working-template/doss.d3 properties/doss_01/
cp /path/to/working-template/properties.qsub properties/doss_01/
cd properties/doss_01
```

Set the total or projected curves described in your projection table. Record the k-point sampling, energy range and broadening settings in `parent-record.txt`. Compare the template with the edited file:

```bash
diff -u /path/to/working-template/doss.d3 doss.d3
grep -n "runcryP\|runpropP\|properties" properties.qsub
qsub properties.qsub
qstat -u "$USER"
```

After the job leaves the queue, inspect the output before plotting:

```bash
ls -rtl
grep -n -i "error\|warning\|doss\|density" doss.out
tail -n 80 doss.out
```

Use the actual output and restart filenames created by your local template. Run related projections from the same parent wavefunction when you need a direct comparison. Changing the parent method or structure creates a different calculation, not a different curve set.
