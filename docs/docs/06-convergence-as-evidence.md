...
Choose levels that suit the model. Keep the functional, basis, geometry, charge and k-point path fixed unless you are explicitly comparing methods. The official [CRYSTAL Basis Sets Library](https://www.crystal.unito.it/basis_sets.html) is a source for available basis definitions, but the calculation itself must show that the selected basis is adequate for your observable.[^1]

## 🗂️ Build the runs

Give each tested level its own directory. For a three-level k-point test:

```bash
mkdir -p kpoint_test/{level_01,level_02,level_03}
cp baseline.d12 kpoint_test/level_01/run.d12
cp baseline.d12 kpoint_test/level_02/run.d12
cp baseline.d12 kpoint_test/level_03/run.d12
```

Edit only the tested setting in `level_02` and `level_03`. Check the changes before submitting:

```bash
diff -u kpoint_test/level_01/run.d12 kpoint_test/level_02/run.d12
diff -u kpoint_test/level_02/run.d12 kpoint_test/level_03/run.d12
```

Copy the approved submission template into each directory when your workflow requires one. Submit each run from its own directory and record every job ID. After completion, apply the same search to every output:

```bash
grep -n -i "converg\|total energy\|final energy" kpoint_test/level_*/run.out
```

Open each matching section with `less` before copying values into the decision table. The grep result locates evidence; it is not a parser and may find intermediate values.
...
[^1]: CRYSTAL. (n.d.). *Basis Sets Library*. https://www.crystal.unito.it/basis_sets.html
